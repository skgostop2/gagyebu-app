...
### 배포 후 추가 변경 — 남은 지연의 구조적 원인: Vercel-Supabase 리전 불일치
미들웨어까지 고쳤는데도 사용자가 "여전히 느리다"고 재확인 → 남은 원인은 리전이었다. Vercel 신규 프로젝트는 기본적으로 미국 동부(`iad1`)에서 서버 함수를 실행하는데, Supabase 프로젝트는 서울(`ap-northeast-2`)에 있다. `getAuthUser()`·`getCurrentMembership()`·각 페이지의 데이터 조회가 전부 서버 함수 안에서 실행되므로, 지금까지 아무리 "중복 호출"을 줄여도 남은 호출 하나하나가 태평양을 왕복하는 지연은 그대로였던 것.
- 저장소 루트에 `vercel.json` 추가(`{"regions": ["icn1"]}`) — Vercel 서버 함수 실행 리전을 서울로 지정해 Supabase와 물리적으로 가깝게 맞췄다. Hobby 플랜은 리전 1개만 선택 가능한데 이전엔 그 1개가 기본값(`iad1`, 미국 동부)이었던 것 — Vercel 공식 문서로 Hobby 플랜도 `icn1` 지정이 가능함을 확인.
- `(dashboard)/loading.tsx` 추가 — Next.js App Router의 자동 Suspense 경계로, 클릭 즉시(데이터 조회가 끝나기 전에) 스켈레톤이 먼저 그려지도록 해 "눌러도 반응이 없어서 다시 누르게 되는" 체감 문제를 완화했다.
- `/tmp/verify4`에서 tsc·eslint(0 경고)·vitest 통과, `npm run build`도 32개 라우트 전부 정상 생성 확인 후 커밋·푸시함.
- 리전 변경은 다음 배포부터 적용되므로(Vercel이 이 커밋을 자동 재배포하면서 반영), 실제 체감 개선은 이번 푸시 이후부터다.
